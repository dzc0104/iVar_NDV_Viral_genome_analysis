########To find the shared amino acid changed (nSNPs)
...
for (isolate in isolates) {
  ...
  if (file.exists(file_p1) && file.exists(file_p10)) {
    ...
    # Remove rows with NA values and where REF_AA is the same as ALT_AA
    p1_aa <- na.omit(p1_aa[p1_aa$REF_AA != p1_aa$ALT_AA, ])
    p10_aa <- na.omit(p10_aa[p10_aa$REF_AA != p10_aa$ALT_AA, ])
